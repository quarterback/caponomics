...
export function LeagueSummary({ s }: { s: Summary }) {
  ...
  return (
    <div className="lsummary">
      ...
      {stat('In tax', `${s.inTaxCount}`, undefined, `owe ${fmtMoney(s.totalTax)}`)}
      {apronTotal > 0 && stat('Over an apron', `${apronTotal}`, 'warn', apronDetail(s.apronCounts))}
      {stat('Payroll spread', `${fmtMoney(s.payrollMin)}–${fmtMoney(s.payrollMax)}`, undefined, `avg ${fmtMoney(s.payrollAvg)}`)}
